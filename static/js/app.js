...
//Get jsonData
let jsonData = '/api/v1.0/teams';
let jsonPlotData = '/api/v1.0/stats';
...
function createMarkers() {
    d3.json(jsonData).then((data) => {
        ...
        for (x in teams) {
            ...
            let sMarker = L.marker([teams[x].StadiumDetails.GeoLat, teams[x].StadiumDetails.GeoLong], { icon: Icon })
                .bindPopup("<h3>" + teams[x].StadiumDetails.Name + "<h3><h3>Capacity: " + teams[x].StadiumDetails.Capacity + "</h3><h3>Type: " + teams[x].StadiumDetails.Type + "</h3>");
            //console.log(sMarker);    
...
function plotdata(selectedTeam, PrimaryColor) {
    ...
    d3.json(jsonPlotData).then((data) => {
        let teamsstats = data.data;
        //console.log(teamsstats)
        for (x in teamsstats) {
            if (selectedTeam == teamsstats[x].Team) {
